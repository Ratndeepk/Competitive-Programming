def Bottomviewutil(root,ans,move,level):
    if root==None:
        return 
    ...
    if move not in ans:
        ...
        ans[move]=[root.data,level]
    Bottomviewutil(root.left,ans,move-1,level+1)
    Bottomviewutil(root.right,ans,move+1,level+1)

def Bottomview(root):
    
    
    ans={}
    Bottomviewutil(root,ans,0,0)
